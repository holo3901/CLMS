...
func (h *HomeController) Get() {
	...
	// 消息通知,发送消息，使用定时任务优化
	qs1 := o.QueryTable("sys_labs_apply")
	labs_apply := []auth.LabsApply{}
	qs1.Filter("user_id", user_id.(int)).Filter("return_status", 0).Filter("notify_tag", 0).All(&labs_apply)

	cur_time, _ := time.Parse("2006-01-02", time.Now().Format("2006-01-02"))

	for _, apply := range labs_apply {
		return_date := apply.ReturnDate
		ret := cur_time.Sub(return_date)
		content := fmt.Sprintf("%s用户，你借的实验室归还时间为%v,已经预期，请尽快归还!!", user.UserName, return_date.Format("2006-01-02"))
		if ret > 0 { // 已经逾期
	...
	// 消息通知,发送消息，使用定时任务优化
	qs1 = o.QueryTable("sys_equips_apply")
	equips_apply := []auth.EquipsApply{}
	qs1.Filter("user_id", user_id.(int)).Filter("return_status", 0).Filter("notify_tag", 0).All(&equips_apply)

	cur_time, _ = time.Parse("2006-01-02", time.Now().Format("2006-01-02"))

	for _, apply := range equips_apply {
		return_date := apply.ReturnDate
		ret := cur_time.Sub(return_date)
		content := fmt.Sprintf("%s用户，你借的器材归还时间为%v,已经预期，请尽快归还!!", user.UserName, return_date.Format("2006-01-02"))
		if ret > 0 { // 已经逾期
			message_notify := auth.MessageNotify{
				Flag:    1,
				Title:   "器材归还逾期",
	...
	qs3 := o.QueryTable("sys_news")
	news := []news.News{}
	qs3.Filter("return_status", 0).Filter("notify_tag", 0).All(&news)
	for _, apply := range news {
...
func (h *HomeController) NotifyList() {
	...
	if kw != "" { // 有查询条件的
		// 总数
		count, _ = qs.Filter("title__contains", kw).Filter("read_tag", 0).Count()
		qs.Filter("title__contains", kw).Filter("read_tag", 0).Limit(pagePerNum).Offset(offsetNum).All(&nofities)
	} else {
		count, _ = qs.Count()
		qs.Limit(pagePerNum).Filter("read_tag", 0).Offset(offsetNum).All(&nofities)
