...
type EquipsBrandController struct {
	beego.Controller
}

func (c *EquipsBrandController) Get() {

	o := orm.NewOrm()

	qs := o.QueryTable("sys_equips_use")

	equips_u := []auth.EquipsUse{}
	...
	if kw != "" { // 有查询条件的
		...
		qs.Filter("is_delete", 0).Filter("name__contains", kw).Limit(pagePerNum).Offset(offsetNum).All(&equips_u)
	} else {
		count, _ = qs.Filter("is_delete", 0).Count()
		qs.Filter("is_delete", 0).Limit(pagePerNum).Offset(offsetNum).All(&equips_u)
	...
	c.Data["kw"] = kw
	c.TplName = "equips/equips_brand_list.html"

}

func (c *EquipsBrandController) ToAdd() {
	c.TplName = "equips/equips_brand_add.html"

}

func (c *EquipsBrandController) DoAdd() {
...
func (u *EquipsBrandController) MuliDelete() {
...
func (u *EquipsBrandController) Delete() {
	id, _ := u.GetInt("id")

	o := orm.NewOrm()
	o.QueryTable("sys_equips_use").Filter("id", id).Update(orm.Params{
		"is_delete": 1,
	})
	ret := fmt.Sprintf("用户id:%d,删除成功", id)
	logs.Info(ret)
	u.Redirect(beego.URLFor("EquipsBrandController.Get"), 302)

}

func (u *EquipsBrandController) ToUpdate() {
	id, _ := u.GetInt("id")
	o := orm.NewOrm()
	user_data := auth.EquipsUse{}
	o.QueryTable("sys_equips_use").Filter("id", id).One(&user_data)
	u.Data["equipsuse"] = user_data
	ret := fmt.Sprintf("用户信息修改，用户id:%d", id)
	logs.Info(ret)
	u.TplName = "equips/equips_brand_edit.html"
}

func (u *EquipsBrandController) DoUpdate() {
	uid, _ := u.GetInt("uid")
	equipsusename := u.GetString("equipsusename")
	desc := u.GetString("desc")
	o := orm.NewOrm()
	qs := o.QueryTable("sys_equips_use").Filter("id", uid)
...
func (u *EquipsBrandController) IsActive() {
	is_active, _ := u.GetInt("is_active_val")
	id, _ := u.GetInt("id")
	o := orm.NewOrm()
	qs := o.QueryTable("sys_equips_use").Filter("id", id)
