...
func (u *NewsController) Delete() {
	id, _ := u.GetInt("id")
	o := orm.NewOrm()
	o.QueryTable("sys_news").Filter("id", id).Update(orm.Params{
		"is_delete": 1,
	})
	ret := fmt.Sprintf("用户id:%d,删除成功", id)
	logs.Info(ret)
	u.Redirect(beego.URLFor("NewsController.Get"), 302)

}

func (u *NewsController) MuliDelete() {
	...
	ret := fmt.Sprintf("批量删除成功，新闻ids:%d", ids)
	logs.Info(ret)
